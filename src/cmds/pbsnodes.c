...
static int job_summary = 0;
static int long_summary = 0;
static int prt_summary = 0;
static mgr_operation_t oper = LISTSP;
static int do_vnodes = 0;
...
int display_pbsnodes(int, int);
struct timeval last_stat_ts = {0, 0};
static int send_opts(int);
static int recv_opts(int);
static char cmd_name[] = "pbsnodes";
static int update_cache(struct batch_status *);

int pbsnodes_qualify(char *asked, int op, struct batch_status *cand);
...
/**
 * @brief
 *	helper function to check error after a call to pbs_statvnode
 *
 * @param[in]  con - server connection fd
 * @param[in]  bstat_head - array head of the response
 *
 * @return  int
 * @retval  0 - success
 ...
 */
int 
check_error(int con, struct batch_status *bstat_head)
{
	char *errmsg;
	if (bstat_head == NULL) {
		if (pbs_errno) {
			if (!quiet) {
				if ((errmsg = pbs_geterrmsg(con)) != NULL)
					fprintf(stderr, "pbsnodes: %s\n", errmsg);
				else
					fprintf(stderr, "pbsnodes: Error %d\n", pbs_errno);
			}
			return (1);
		} else if (!cc_get_head()) {
			if (!quiet)
				fprintf(stderr, "pbsnodes: No nodes found\n");
			return (0);
		}
	}
	return (0);
...
/*
 * brief In case of JSON format requested, add a json prologue 
 */
void
add_json_prologue()
{
...
main(int argc, char *argv[])
{
	...
	if (oper == ALL || oper == LISTMRK || oper == LISTSP || oper == LISTSPNV) {
		set_diff_stat = 1;
		if (oper == LISTSP || oper == LISTSPNV) {
			/* allocate enough memory to store list of job ids */
			if (!(node_list = malloc((argc - 1) * (PBS_MAXHOSTNAME + 1)))) {
	...
	if (set_diff_stat) {
		...
		rc = check_error(con, bstat_head);
		if (rc != 0)
			exit(rc);

		update_cache(bstat_head);
	} else {
	...
	switch (oper) {
		...
		case LISTSP:
		case LISTSPNV:
			rc = display_pbsnodes(con, set_diff_stat);
			break;
	}

	if (set_diff_stat)	
		go_bg(cmd_name, fname, con, &talk_to_fg);
	else 
		pbs_disconnect(con);
...
update_cache(struct batch_status *stat_upd)
{
	...
	while(stat_upd) {
		...
		if (full_stat)
			...
			rc = cc_delete(stat_upd);
		else
			rc = cc_update(stat_upd);
		
		if (rc != 0) {
			fprintf(stderr, "Failed to add/update/delete node %s in index\n", stat_upd->name);
			fprintf(stderr, "Out of memory\n");
			exit(1);
		}
		stat_upd = next;			
 ...
 * @param[in] query_nodename - node name queried or NULL for all
 *
 */
static void
add_nodes_from_cache(struct batch_status **phead, char *query_nodename)
{
	struct batch_status *t_status = NULL;

	while ((t_status = cc_get_next(query_nodename, t_status))) {
		if (cc_add_list(phead, t_status) != 0) {
			fprintf(stderr, "Out of memory\n");
			exit(1);
		}
	}
 ...
 *
 * @param[in] query_nodename_list - list of node names queried, or NULL for all
 * 
 * @return batch_status - List of qualifying jobs in batch_status list format
 *
 */
static struct batch_status *
build_resp_from_cache(char *query_nodename_list)
{
	struct batch_status *phead = NULL;
	char *name;
	char *p_querylist = NULL;
	
	if (!query_nodename_list) {
		add_nodes_from_cache(&phead, NULL);
	} else {
		name = strtok_r(query_nodename_list, ",", &p_querylist);
		while (name) {
			add_nodes_from_cache(&phead, name);
			name = strtok_r(NULL, ",", &p_querylist);
		}

		/* if names were given and not found, set pbs_errno */
		if (!phead)
			pbs_errno = PBSE_UNKNODE;
	}

#ifdef CLI_DEBUG
	CLI_DBPRT((stderr, "After build_resp_from_cache()\n"))
...
/**
 * @brief
 *	The top level routine to display the pbsnodes output
 *
 * @param[in] sd_svr - the connection descriptor to the server
 * @param[in] set_diff_set - was diff stat to be used?
 * 
 * @return - error code
 * @retval 0 - success
 * @retval !0 - error code 
 ...
 */
int 
display_pbsnodes(int sd_svr, int set_diff_stat)
{
	struct batch_status *p_status = NULL, *bstat;
	struct attrl *pattr = NULL;
	struct batch_status *next_bstat = NULL;
	struct batch_status *hstat = NULL;
	char *pa;
	char *p_nodelist = NULL;
	int rc = 0;
	...
	switch (oper) {
		case ALL:
			if (!(do_vnodes || oper == LISTSPNV)) {
				p_status = build_resp_from_cache(NULL); /*make_stathost_return munges the attributes, so need to make a copy */
				hstat = make_stathost_return(sd_svr, "", p_status);
				pbs_statfree(p_status); /* this was a deep copy, so can free */
				p_status = hstat;
			} else
				p_status = build_resp_from_cache(NULL);

			if (prt_summary) {
				if (prt_node_summary(def_server, p_status, job_summary, long_summary)) {
					fprintf(stderr, "pbsnodes: out of memory\n");
					return 1;
				}
			} else {
				for (bstat = p_status; bstat; bstat = bstat->next)
					prt_node(bstat);
			}
			if (output_format == FORMAT_JSON) {
				if (add_json_node(JSON_OBJECT_END, JSON_NULL, JSON_NOVALUE, NULL, NULL) == NULL) {
		...
		case LISTSP:
			...
			pa = strtok_r(node_list, ",", &p_nodelist);
			while (pa) {
				if (do_vnodes) {
					p_status = build_resp_from_cache(pa);
					bstat = p_status;
				} else {  /* make stathost type return from vnodes */
					p_status = build_resp_from_cache(NULL); /* make_stathost_return munges the attributes, so need to make a copy */
					hstat = make_stathost_return(sd_svr, pa, p_status);
					pbs_statfree(p_status); /* we made a copy, so can free */
					bstat = hstat;
				}
				if (!bstat) {
					if (pbs_errno != 0) {
						if (output_format == FORMAT_JSON) {
							...
							add_json_node(JSON_OBJECT_END, JSON_STRING, JSON_FULLESCAPE, "Error", pbse_to_txt(pbs_errno));
						} else {
							fprintf(stderr, "Node: %s,  Error: %s\n", pa, pbse_to_txt(pbs_errno));
							rc = 1;
						}
					}
				} else {
 ...
 *
 * @return error code
 * @retval -1 - error in communiaction with foreground
 * @retval !=-1 - success(0) or error code from server
 *
 */
int
talk_to_fg(int sock, int sd_svr, char **err_op)
{
	...
	struct batch_status *bstat_update;
	...
	if (((outfd = recv_fd(sock)) == -1) ||
	    ...
	    (recv_dyn_string(sock, &node_list) != 0) ||
	    (recv_opts(sock) != 0)) {
	...
	if (now - last_time >= STAT_REFRESH_INTERVAL) {
		...
		sprintf(buf, ",%ld:%ld", last_stat_ts.tv_sec, last_stat_ts.tv_usec);
		bstat_update = pbs_statvnode(sd_svr, "", NULL, buf);
		rc = check_error(sd_svr, bstat_update);
		if (bstat_update)
			update_cache(bstat_update);
	}

	if ((rc == 0) && cc_get_head()) {
		rc = display_pbsnodes(sd_svr, 1);
	}
...
{
	char buf[20];
	sprintf(buf, "%d %d %d %d %d %d %d", oper, job_summary, long_summary, output_format, prt_summary, do_vnodes, quiet);
...
recv_opts(int sock)
{
	...
	sscanf(buf, "%d %d %d %d %d %d %d", (int *) &oper, &job_summary, &long_summary, &output_format, &prt_summary, &do_vnodes, &quiet);
	free(buf);
