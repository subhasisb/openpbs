		...
		case TPP_AUTH_CTX: {
			...
			if (authdata == NULL) {
				...
				if (strcmp(ahdr.auth_method, AUTH_RESVPORT_NAME) != 0 &&
					...
					snprintf(msg, TPP_GEN_BUF_SZ, "tfd=%d, Authentication method not supported in connection %s", tfd, tpp_netaddr(&connected_host));
				...
				if (ahdr.encrypt_method[0] != '\0' && get_auth(ahdr.encrypt_method) == NULL) {
					snprintf(msg, TPP_GEN_BUF_SZ, "tfd=%d, Encryption method not supported in connection %s", tfd, tpp_netaddr(&connected_host));
					tpp_log(LOG_CRIT, NULL, msg);
					tpp_send_ctl_msg(tfd, TPP_MSG_AUTHERR, &connected_host, &this_router->router_addr, -1, 0, msg);
					return 0; /* let connection be alive, so we can send error */
				}
			...
			if (ctx == NULL) { /* connection not yet authenticated */
				if (extra && strcmp(((conn_auth_t *)extra)->config->auth_method, AUTH_RESVPORT_NAME) != 0) {
					...
					if (tpp_transport_isresvport(tfd) != 0) {
						snprintf(msg, TPP_GEN_BUF_SZ, "Connection from non-reserved port, rejected");
						tpp_log(LOG_CRIT, NULL, msg);
						tpp_send_ctl_msg(tfd, TPP_MSG_AUTHERR, &connected_host, &this_router->router_addr, -1, 0, msg);
						return 0; /* let connection be alive, so we can send error */
					}
