...
internal_tcp_connect(int channel, char *server, int port, char *extend_data)
{
	...
	fprintf(stderr, "Trying host=%s:%d, ", server, port);
...
tryagain:
		if (first_index > -1) {
			...
			if (srv_index == first_index) {
				fprintf(stderr, "No nonfailed server, bailing out\n");
		...
		fprintf(stderr, "Selected server index %d, srv=%s:%d, state=%d, ", srv_index, pbs_conf.psi[srv_index]->name, pbs_conf.psi[srv_index]->port, connection[channel].ch_shards[srv_index]->state);
