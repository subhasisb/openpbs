...
mgr_unset_attr(attribute *pattr, attribute_def *pdef, int limit, svrattrl *plist, int privil, int *bad, void *pobj, int ptype, enum res_op_flag rflag)
{
	...
	while (plist) {
		...
		if (encode_an_attr_db((pdef + index), (pattr + index), &cache_attr_list, &db_attr_list, 0) != 0)
			return (PBSE_NOATTR);
							...
							if (i >= 0) {
								if ((pattr+i)->at_flags & ATR_VFLAG_SET) {
									...
									if ((nresc = find_resc_entry((pattr+i), prsdef)) != NULL) {
										...
										(pattr+i)->at_flags |= ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
									}
								}
							}
						}
	...
	switch (ptype) {
			...
			case PARENT_TYPE_QUE_ALL:
				...
				sv_time = ((pbs_queue *) pobj)->qu_mtime;
...
mgr_sched_create(struct batch_request *preq)
{
	...
	psched->sc_newsched = 1; /* since new sched object is being created */
