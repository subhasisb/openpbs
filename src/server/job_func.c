...
/**
 * @brief
 * 		load a job from database and initialize it
 *
 * @param[in]	jobid - job ID string.
 *
 * @return	pointer to job struct
 ...
 */
job * 
load_and_init_job(char *jobid, job *pjob)
{
	extern int server_init_type;

	if (pjob == NULL) {
		if (!(pjob = job_recov_db(jobid, NULL))) {
			if ((server_init_type == RECOV_COLD) || (server_init_type == RECOV_CREATE)) {
				...
				if (pbs_db_delete_obj(svr_db_conn, &obj) != 0) {
					sprintf(log_buffer, "Failed to purge job %s", jobid);
					log_err(-1, __func__, log_buffer);
				}
			} else {
				sprintf(log_buffer, "Failed to recover job %s", jobid);
...
find_job(char *jobid)
{
	...
	if (pj == NULL || pj->initialized == 0) {
		pj = load_and_init_job(jobid, pj);
	}
	return pj;
#endif
