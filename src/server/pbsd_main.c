...
main(int argc, char **argv)
{
	...
	if (get_max_servers() > 1) {
		sprintf(lockfile, "%s/%s/server_%d.lock", pbs_conf.pbs_home_path, PBS_SVR_PRIVATE, pbs_conf.batch_service_port);
	} else {
		sprintf(lockfile, "%s/%s/server.lock", pbs_conf.pbs_home_path, PBS_SVR_PRIVATE);
	}
