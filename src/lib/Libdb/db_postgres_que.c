...
pg_db_prepare_que_sqls(pbs_db_conn_t *conn)
{
	snprintf(conn->conn_sql, MAX_SQL_LENGTH, "insert into pbs.queue("
		...
		"returning to_char(qu_mtime, 'YYYY-MM-DD HH24:MI:SS.US') as qu_mtime");
	...
	snprintf(conn->conn_sql, MAX_SQL_LENGTH, "select qu_name, "
			"qu_type, "
			"to_char(qu_ctime, 'YYYY-MM-DD HH24:MI:SS.US') as qu_ctime, "
			"to_char(qu_mtime, 'YYYY-MM-DD HH24:MI:SS.US') as qu_mtime, "
...
load_que(PGresult *res, pbs_db_que_info_t *pq, int row)
{
	...
	GET_PARAM_STR(res, row, pq->qu_ctime, qu_ctime_fnum);
	GET_PARAM_STR(res, row, pq->qu_mtime, qu_mtime_fnum);
...
pg_db_save_que(pbs_db_conn_t *conn, pbs_db_obj_info_t *obj, int savetype)
{
	...
	if (stmt != NULL) {
		...
		GET_PARAM_STR(conn->conn_resultset, 0, pq->qu_mtime, qu_mtime_fnum);
...
pg_db_load_que(pbs_db_conn_t *conn, pbs_db_obj_info_t *obj)
{
	...
	if (rc == 0) {
		...
		//if (use_dist_cache) {
		//	dist_cache_recov_attrs(pq->qu_name, &pq->qu_mtime, &pq->cache_attr_list);
		//}
	}
...
{
	pg_query_state_t *state = (pg_query_state_t *) st;
	obj->pbs_db_un.pbs_db_que->qu_mtime[0] = '\0';
...
pg_db_del_attr_que(pbs_db_conn_t *conn, void *obj_id, char *sv_time, pbs_db_attr_list_t *attr_list)
{
	...
	GET_PARAM_STR(conn->conn_resultset, 0, sv_time, qu_mtime_fnum);
	PQclear(conn->conn_resultset);
	free(raw_array);
