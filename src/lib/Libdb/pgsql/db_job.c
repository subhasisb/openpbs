...
pbs_db_save_job(void *conn, pbs_db_obj_info_t *obj, int savetype)
{
	...
	if (stmt != NULL) {
		...
		GET_PARAM_STR(res, 0, pjob->ji_savetm, ji_savetm_fnum);
		PQclear(res);
		free(raw_array);
	}
 ...
 */
int
pbs_db_del_attr_job(void *conn, void *obj_id, char *sv_time, pbs_db_attr_list_t *attr_list)
{
	char *raw_array = NULL;
	int len = 0;
	static int ji_savetm_fnum;
	...
	GET_PARAM_STR(res, 0, sv_time, ji_savetm_fnum);
	PQclear(res);
	free(raw_array);
