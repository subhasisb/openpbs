...
svr_to_db_que(pbs_queue *pque, pbs_db_que_info_t *pdbque)
{
	...
	strcpy(pdbque->qu_mtime, pque->qu_mtime);
...
db_to_svr_que(pbs_queue *pque, pbs_db_que_info_t *pdbque)
{
	...
	strcpy(pque->qu_mtime, pdbque->qu_mtime);
...
que_save_db(pbs_queue *pque)
{
	...
	if ((savetype = svr_to_db_que(pque, &dbque)) == -1)
		goto done;
	...
	if (pbs_db_save_obj(conn, &obj, savetype) == 0) {
		strcpy(pque->qu_mtime, dbque.qu_mtime);
		pque->qu_newque = 0;
		rc = 0;
	}
...
que_recov_db(char *qname, pbs_queue	*pq)
{
	...
	if (pq)
		strcpy(dbque.qu_mtime, pq->qu_mtime);
	else {
		dbque.qu_mtime[0] = '\0';
