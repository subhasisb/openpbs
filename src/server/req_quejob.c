...
req_quejob(struct batch_request *preq)
{
	...
	/* for server, no need to check if job exists, save would fail if already exists */

	/* TODO_MSVR: Remove the need to do a avl_search here */
	if ((svr_chk_history_conf()) && (pj = find_job_avl(jid))) {
		/*
		...
		* exists.
		*/
		if ((pj->ji_qs.ji_state == JOB_STATE_MOVED)) {
			job_purge(pj);
		} else {
...
req_commit_now(struct batch_request *preq, job *pj)
{
	...
	count = 0;
	do {
		if ((rc = job_save_db(pj)) == 1) {
			/* 
			 * we circled and there is a job-id clash 
			...
			if (strchr(pj->ji_qs.ji_jobid, '[') == NULL) {	/* Normal job */
				(void)sprintf(pj->ji_qs.ji_jobid, "%lld.%s", next_svr_sequence_id, server_name);
			} else {	/* Array Job */
				(void)sprintf(pj->ji_qs.ji_jobid, "%lld[].%s", next_svr_sequence_id, server_name);
			}
			count++;
	...
	if (count > 0) {
		sprintf(log_buffer, "spinned %d times trying to find a non-clash jobid", count);
	...
	pj->initialized = 1; /* very important */
