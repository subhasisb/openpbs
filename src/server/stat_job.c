...
int status_attrib(svrattrl *pal, attribute_arr *parr, int limit, int priv, pbs_list_head *phead, int *bad)
{
	int index;
	int nth = 0;
	...
	{ /* non specified, return all readable attributes */
...
int status_job(job *pjob, struct batch_request *preq, svrattrl *pal, pbs_list_head *pstathd, int *bad, int dosubjobs)
{
	...
	*bad = 0;
	if (status_attrib(pal, &pjob->ji_wattr, pjob->ji_wattr.defn->count, preq->rq_perm, &pstat->brp_attr, bad))
		return (PBSE_NOATTR);
 ...
 */
int status_subjob(job *pjob, struct batch_request *preq, svrattrl *pal, int subj, pbs_list_head *pstathd, int *bad, int dosubjobs)
{
	int limit = pjob->ji_wattr.defn->count;
	struct brp_status *pstat;
	...
	*bad = 0;
	...
	if (status_attrib(pal, &pjob->ji_wattr, limit, preq->rq_perm, &pstat->brp_attr, bad))
		rc = PBSE_NOATTR;
