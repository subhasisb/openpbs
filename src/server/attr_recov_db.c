...
obj_qs_modified(void *qs, int len, void *oldhash)
{
	...
	if (memcmp(hash, oldhash, SHA_DIGEST_LENGTH) != 0) {
		memcpy(oldhash, hash, SHA_DIGEST_LENGTH); /* update the signature */
		return 1;
	}
 ...
 */
int 
encode_an_attr_db(struct attribute_def *padef, struct attribute *pattr, pbs_db_attr_list_t *cache_attr_list, pbs_db_attr_list_t *db_attr_list, int all)
{
	pbs_list_head *lhead;
	int rc = 0;
	int chkflag = 0;
...
encode_attr_db(struct attribute_def *padef, struct attribute *pattr, int numattr, pbs_db_attr_list_t *cache_attr_list, pbs_db_attr_list_t *db_attr_list, int all)
{
	...
	for (i = 0; i < numattr; i++) {
		...
		if (encode_an_attr_db((padef + i), (pattr + i), cache_attr_list, db_attr_list, all) != 0)
			return -1;
