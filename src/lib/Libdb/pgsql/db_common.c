...
db_prepare_sqls(void *conn)
{
	if (pbs_db_prepare_job_sqls(conn) != 0)
		return -1;
	if (pbs_db_prepare_svr_sqls(conn) != 0)
		return -1;
	if (pbs_db_prepare_que_sqls(conn) != 0)
		return -1;
	if (pbs_db_prepare_resv_sqls(conn) != 0)
		return -1;
	if (pbs_db_prepare_node_sqls(conn) != 0)
		return -1;
	if (pbs_db_prepare_sched_sqls(conn) != 0)
		return -1;
	return 0;
}
 ...
 * @param[in]	sv_time - Object save time.
 * @param[in]	cache_attr_list - list of attributes to delete from cache.
 * @param[in]	db_attr_list - list of attributes to delete from db.
 *
 * @return      Error code
 * @retval      0  - success
 * @retval     -1  - Failure
 ...
 */
int
pbs_db_delete_attr_obj(void *conn, pbs_db_obj_info_t *obj, void *obj_id, char *sv_time, pbs_db_attr_list_t *cache_attr_list, pbs_db_attr_list_t *db_attr_list) 
{
	...
	return (db_fn_arr[obj->pbs_db_obj_type].pbs_db_del_attr_obj(conn, obj_id, sv_time, db_attr_list));
}
 ...
 *			functionality.
 * @param[in]	msg - SQL statement executed.
 * @param[in]	msg2 - Custom string added to the error message. This can be
 ...
 * @param[in]	num_vars - The number of parameters in the sql ($1, $2 etc)
 * @param[out]  res_out - The result set of the query
 *
 * @return      Error code
...
db_cmd(void *conn, char *stmt, int num_vars, PGresult **res_out)
{
	...
	if (res_out != NULL)
		*res_out = res; /* store, since caller will retrieve results */
	else
		PQclear(res);
...
pbs_db_get_errmsg(int err_code, char **err_msg)
{
	...
	case PBS_DB_INIT_FAIL:
		*err_msg = strdup("Failed to initialize PBS dataservice");
		break;
