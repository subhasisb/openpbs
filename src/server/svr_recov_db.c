 ...
 */
static int
svr_to_db_sched(struct pbs_sched *ps, pbs_db_sched_info_t *pdbsched)
{
	int savetype = 0;
...
svr_save_db(struct server *ps)
{
	...
	if ((savetype = svr_to_db_svr(ps, &dbsvr)) == -1)
		goto done;
...
sched_save_db(pbs_sched *ps)
{
	...
	if ((savetype = svr_to_db_sched(ps, &dbsched)) == -1)
		goto done;
	...
	if (rc != 0) {
		strcpy(log_buffer, "Unable to recover sched");
		if (conn->conn_db_err != NULL)
			strncat(log_buffer, conn->conn_db_err, LOG_BUF_SIZE - strlen(log_buffer) - 1);
		log_err(-1, __func__, log_buffer);
