...
pg_db_prepare_svr_sqls(pbs_db_conn_t *conn)
{
	...
	snprintf(conn->conn_sql, MAX_SQL_LENGTH, "insert into pbs.serverinst( "
		"sv_id, "
		"sv_jobidnumber "
		") "
		"values "
		"($1, $2)");
	if (pg_prepare_stmt(conn, STMT_INSERT_SVRINST, conn->conn_sql, 2) != 0)
		return -1;	

	snprintf(conn->conn_sql, MAX_SQL_LENGTH, "update pbs.serverinst set "
		"sv_jobidnumber = $2 "
		" where sv_id = $1");
	if (pg_prepare_stmt(conn, STMT_UPDATE_SVRINST, conn->conn_sql, 2) != 0)
		return -1;	

	snprintf(conn->conn_sql, MAX_SQL_LENGTH, "select "
		"COALESCE(max(sv_jobidnumber), 0) as sv_jobidnumber "
		"from "
		"pbs.serverinst");
	if (pg_prepare_stmt(conn, STMT_SELECT_SVRINST, conn->conn_sql, 0) != 0)
		return -1;
...
int 
pg_db_save_instdata(pbs_db_conn_t *conn, char *sv_id, long long sv_jobidnumber)
{
	char *stmt;
	int params;

	SET_PARAM_STR(conn, sv_id, 0);
	SET_PARAM_BIGINT(conn, sv_jobidnumber, 1);
	params = 2;
	stmt = STMT_UPDATE_SVRINST;

	if (pg_db_cmd(conn, stmt, params) != 0) {
		stmt = STMT_INSERT_SVRINST;
		if (pg_db_cmd(conn, stmt, params) != 0)
			return -1;
	}
...
int
pg_db_load_maxid(pbs_db_conn_t *conn, long long *sv_jobidnumber)
{
	PGresult *res;
	int rc;

	if ((rc = pg_db_query(conn, STMT_SELECT_SVRINST, 0, &res)) != 0)
		return -1;

	GET_PARAM_BIGINT(res, 0, *sv_jobidnumber, 0);
	PQclear(res);
