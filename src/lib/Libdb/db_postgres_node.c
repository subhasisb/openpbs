...
pg_db_load_nodejob(pbs_db_conn_t *conn, pbs_db_obj_info_t *obj, int lock)
{
	...
	if ((rc = pg_db_query(conn, STMT_SELECT_NODEJOB, 2, lock, &res)) == -1)
		return -1;

	/* no rows returned */
	if (rc == 1) {
		strncpy(pnj->job_id, obj->pbs_db_un.pbs_db_nodejob->job_id, PBS_MAXSVRJOBID);
		strncpy(pnj->nd_name, obj->pbs_db_un.pbs_db_nodejob->nd_name, PBS_MAXSERVERNAME);
		/* we set this value to convey the caller that we could not find any corresponding entries */
		pnj->is_resv = 2;
		pnj->admn_suspend = 0;
		pnj->subnode_ct = -1;
		return 0;
